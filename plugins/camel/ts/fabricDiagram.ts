    ...
    $scope.$watch("selectedNode", (newValue, oldValue) => {
      ...
      if (node) {
        ...
        if (mbean && nodeJolokia) {
          ...
          }, Core.onSuccess(renderNodeAttributes, {error: (response) => {
      ...
      if (containerJolokia) {
        ...
        function loadMetaDataFromEndpointMBeans() {
          ...
          if ($scope.viewSettings.route) {
            containerJolokia.request({type: "read", mbean: "org.apache.camel:type=routes,*", attribute: ["EndpointUri"]}, Core.onSuccess((response) => {
          ...
          if ($scope.viewSettings.endpoint) {
            containerJolokia.search("org.apache.camel:type=endpoints,*", Core.onSuccess((response) => {
        ...
        function getOrCreateCamelContext(contextId, contextMBean = null) {
          ...
          if (matchesContextId(contextId)) {
            ...
            containerJolokia.execute(contextMBean, "createRouteStaticEndpointJson", Core.onSuccess((response) => {
        ...
        containerJolokia.search("org.apache.camel:type=context,*", Core.onSuccess((response) => {
          angular.forEach(response, (objectName) => {
            var details = Core.parseMBean(objectName);
            var attributes = details['attributes'];
            var contextId = attributes["context"];
