namespace Diagnostics {
  ...
  export interface JvmFlagsScope extends ng.IScope {
    flags: Array<JvmFlag>;
    tableDef:any;
}

  export function DiagnosticsFlagsController($scope: JvmFlagsScope, jolokia: Jolokia.IJolokia) {
    'ngInject';
      ...
      for (let i = 0; i < $scope.flags.length; i++) {
        ...
        flag.deregisterWatch = $scope.$watch('flags[' + i + ']', (newValue: JvmFlag, oldValue: JvmFlag) => {
          if (newValue.value != oldValue.value) {
            ...
            }, readRequest], Core.onSuccess((response) => {//immediate refresh piggybacked read value
