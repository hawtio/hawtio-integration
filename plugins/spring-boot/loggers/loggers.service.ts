...
namespace SpringBoot {
  ...
  export class LoggersService {
    ...
    getLoggerConfiguration(): ng.IPromise<LoggerConfiguration> {
      const mbean: EndpointMBean = this.springBootService.getEndpointMBean(['Loggers'], ['getLoggers', 'loggers'])
    ...
    setLoggerLevel(logger: Logger): ng.IPromise<void> {
      const mbean: EndpointMBean = this.springBootService.getEndpointMBean(['Loggers'], ['setLogLevel', 'configureLogLevel'])
